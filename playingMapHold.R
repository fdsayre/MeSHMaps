#Playing file
# holding messy stuff from trying to figure out how to make this work
# the following is a direct paste from April 25
...
duitrees <- read.csv("duiToTree.csv") # crap need to fix the below stuff because not pulling in variable names right
# what its doing is pulling in that stupid first column with row numbers

new <- merge(summarydf, duitrees, by.x = "DUI1", by.y = "dui")
colnames(new)
colnames(new)[5] <- "treedui1"
colnames(new)[6] <-"tdui1"

newdf <- merge(new, duitrees, by.x = "DUI2", by.y="dui", allow.cartesian=TRUE) 

# bring in treenumbers

newdftreenumbers <- merge(newdf, treenumbers, by.x = "subt.x", by.y="A", allow.cartesian=TRUE) 


treecorrelations <- newdf %>%
  select(Freq, tdui1, subt.x)

treecorrelations2 <- newdftreenumbers %>% #for tree numbers
  select(Freq, tdui1, Anatomy)
...
# this works! 
library(circlize)
freqpairs <- treecorrelations %>% group_by(tdui1, subt.x) %>%
  summarize(freq = n())
chordDiagram(freqpairs) # works, don't change above

# this works! #trying with second level tree numbers
library(circlize)
freqpairs <- treecorrelations2 %>% group_by(tdui1, Anatomy) %>%
  summarize(freq = n())
chordDiagram(freqpairs) # works, don't change above
...
chordDiagram(freqpairs, annotationTrack = "grid", preAllocateTracks = 1)
circos.trackPlotRegion(track.index = 1, panel.fun = function(x, y) {
  xlim = get.cell.meta.data("xlim")
  ylim = get.cell.meta.data("ylim")
  sector.name = get.cell.meta.data("sector.index")
